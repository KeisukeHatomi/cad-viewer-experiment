...
### 5. DXF MTEXT 書式コードの未除去（重大度: 中）

- **場所**: `src/components/DxfViewer.jsx`
- **説明**: MTEXT エンティティのテキストに含まれる DXF リッチテキスト書式コード（`\f` フォント指定、`\p` 段落設定、`{}` グループ括弧など）を除去せずそのまま CSS2DObject に表示していたため、`{\fBIZ UDGothic|b0;品番}` のような生コードが図面上に表示されていた。
- **影響**: 図面上のテキストが意図した内容ではなく書式コードとして表示される。
- **対応**: ✅ 修正済み — `stripMtext()` 関数を追加し、`\f`, `\p`, `\H`, `\C`, `{}` 等の書式コードを除去した上でプレーンテキストを表示するよう修正。
...
- DXF: LWPOLYLINE の bulge（円弧補間）に未対応。現状は直線補間。
