...
|--------|------|
| Shaded | シェードのみ（エッジなし） |
| Shaded + Edges | シェード＋全エッジ表示（デフォルト） |
| Hidden Line | 陰線消去（背景色でマスク） |
...
| 操作 | 動作 |
|------|------|
| キューブの面をクリック | その面の正投影方向にスナップ（アニメーション付き） |
| ▲ / ▼ / ◀ / ▶ 矢印 | 上面 / 下面 / 左面 / 右面へスナップ |
| 全体表示ボタン | 等角投影ビュー（ISO）にリセット |

スナップ時はモデルのバウンディングボックスに合わせて自動フィット（中心合わせ・サイズ調整）します。
